...
from Splicing.data.dataset_PSData import PSData
    ...
    def __init__(self, crop_size, grid_crop, blocks=('RGB',), mode="train", DCT_channels=3, read_from_jpeg=False, class_weight=None):
        ...
        if mode == "train":
            ...
            self.dataset_list.append(
                PSData(crop_size, grid_crop, blocks, DCT_channels, "Splicing/data/PSData_train_list.txt",
                      read_from_jpeg=read_from_jpeg))
            self.dataset_list.append(
                PSData(crop_size, grid_crop, blocks, DCT_channels, "Splicing/data/PSData_auth_train_list.txt",
                       read_from_jpeg=read_from_jpeg))
            ...
            self.dataset_list.append(
                PSData(crop_size, grid_crop, blocks, DCT_channels, "Splicing/data/PSData_val_list.txt",
                       read_from_jpeg=read_from_jpeg))
            self.dataset_list.append(
                PSData(crop_size, grid_crop, blocks, DCT_channels, "Splicing/data/PSData_auth_val_list.txt",
                       read_from_jpeg=read_from_jpeg))
